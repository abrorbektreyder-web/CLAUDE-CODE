    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
        <div className="premium-card rounded-2xl p-4 md:p-5">
          <div className="mb-2 text-[10px] md:text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)]">Jami mijozlar</div>
          <div className="text-xl md:text-2xl font-bold">{initialData.length} <span className="text-[10px] md:text-xs font-normal text-[var(--color-text-tertiary)]">nafar</span></div>
        </div>
        <div className="premium-card rounded-2xl p-4 md:p-5">
          <div className="mb-2 text-[10px] md:text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)]">Umumiy savdo</div>
          <div className="text-xl md:text-2xl font-bold text-[var(--color-success)]">
        ...
        <div className="premium-card rounded-2xl p-4 md:p-5 sm:col-span-2 md:col-span-1">
          <div className="mb-2 text-[10px] md:text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)]">Umumiy qarzlar</div>
      ...
      <div className="premium-card overflow-hidden rounded-2xl">
              ...
              {filteredData.length > 0 ? filteredData.map((customer) => {
                ...
                return (
                  <tr key={customer.id} className="group transition-colors hover:bg-[var(--color-bg-hover)]">
                    ...
                    <td className="px-6 py-4">
                      {customer.isActive ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-success)]/10 px-2.5 py-0.5 text-[10px] font-bold text-[var(--color-success)] border border-[var(--color-success)]/20">Aktiv</span>
                      ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-bg-elevated)] px-2.5 py-0.5 text-[10px] font-bold text-[var(--color-text-tertiary)] border border-[var(--color-border)]">Bloklangan</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button className="rounded-lg p-2 text-[var(--color-text-tertiary)] hover:bg-[var(--color-bg-elevated)] transition-colors">
        ...
        <div className="md:hidden divide-y divide-[var(--color-border)]">
          {filteredData.length > 0 ? filteredData.map((customer) => {
            ...
            const isDebtor = debt > 0;
            return (
              <div key={customer.id} className="p-4 space-y-4 active:bg-[var(--color-bg-hover)] transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-blue-500/20 text-blue-600">
                      <User size={24} />
                    </div>
                    <div>
                      <div className="font-bold text-base leading-tight mb-0.5">{customer.fullName}</div>
                      <div className="text-[10px] text-[var(--color-text-tertiary)] font-bold uppercase">+998 ** *** *{customer.phoneLastFour}</div>
                    </div>
                  </div>
                  <button className="rounded-lg p-2 text-[var(--color-text-tertiary)] bg-[var(--color-bg-elevated)]">
                    <MoreVertical size={18} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-[10px] font-bold uppercase text-[var(--color-text-tertiary)] mb-0.5">Savdo</div>
                    <div className="font-bold text-base">{formatPrice(customer.totalSpent)} <span className="text-[10px] text-[var(--color-text-tertiary)]">SO'M</span></div>
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase text-[var(--color-text-tertiary)] mb-0.5">Qarzdorlik</div>
                    <div className={cn("font-bold text-base", isDebtor ? "text-[var(--color-danger)]" : "text-[var(--color-text-tertiary)]")}>
                      {formatPrice(customer.totalDebts)} <span className="text-[10px] text-[var(--color-text-tertiary)]">SO'M</span>
                    </div>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-[var(--color-border)]/50">
                  <div className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase">
                    Oxirgi xarid: {formatDate(customer.lastPurchaseAt)}
                  </div>
                  {customer.isActive ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-success)]/10 px-2 py-0.5 text-[9px] font-bold text-[var(--color-success)] border border-[var(--color-success)]/20">Aktiv</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-bg-elevated)] px-2 py-0.5 text-[9px] font-bold text-[var(--color-text-tertiary)] border border-[var(--color-border)]">Bloklangan</span>
                  )}
                </div>
              </div>
            );
